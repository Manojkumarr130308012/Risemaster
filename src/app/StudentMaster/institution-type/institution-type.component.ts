...
export class InstitutionTypeComponent implements OnInit {
  ...
  viewData() {
    this.request.getInstitutionType().subscribe(
      response => {
  ...
  onAddSubmit() {
    ...
    this.request.addInstitutionType(this.registerForm.value).subscribe(
      (res: any) => {
        if (res.status == "error") {
  ...
  onEdit(institutionType) {
    ...
    this.request.fetchInstitutionTypeById(this.Id).subscribe(response => {
      ...
      this.editForm = this.formBuilder.group({
        institutionType2: [this.institutionTypeValue, Validators.required]
      });
  ...
  onEditSubmit() {
    ...
    const edata = {
      institutionType: this.editForm.get("institutionType2").value
    };

    this.request.updateInstitutionType(this.IdValue, edata).subscribe(
      (response: any) => {
