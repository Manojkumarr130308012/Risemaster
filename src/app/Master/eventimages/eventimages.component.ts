...
export class EventimagesComponent implements OnInit {
  ...
  constructor(
    ...
    this.request.addeventimage(edata).subscribe((res: any) => {
...
private saveFileData(fileUpload: FileUpload) {
  ...
  this.request.getaggeventimage(this.id).subscribe((response) => {
    this.eventimageNames = response;
    console.log("sssss",this.eventimageNames);
  }, (error) => {
    console.log(error);
  });
  }
  ...
  deleteBloodgroup(id: any) {
    ...
    Swal.fire({  
      ...
      if (result.value) {   
        this.request.deleteeventimage(id).subscribe(res => {
          console.log(id);
          this.viewData();
        console.log('Deleted');
        });
  ...
  onEdit(bloodgroup){
    ...
    this.request.fetcheventimageById(this.Id).subscribe((response) => {
      this.editTrackNamegroup=response[0];
      console.log(response);
        this.url=this.editTrackNamegroup.eventimage;

        this.IdValue=this.editTrackNamegroup._id;

        this.editForm = this.formBuilder.group({
          TrackName2:[this.url, Validators.required]
      });
  ...
  this.request.updatetracker(this.IdValue,edata1).subscribe((res : any) => {
    if (res.status == 'success') {
