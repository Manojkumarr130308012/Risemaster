...
export class CourseCategoryComponent implements OnInit {
  ...
  onAddSubmit() {
    ...
    this.request.addCoursecategory(this.registerForm.value).subscribe(
      (res: any) => {
  ...
  viewData() {
    this.request.getCoursecategory().subscribe(
      response => {
  ...
  onEditSubmit() {
    ...
    this.request.updateCoursecategory(this.IdValue, edata).subscribe(
      (res: any) => {
        if (res.status == "success") {
