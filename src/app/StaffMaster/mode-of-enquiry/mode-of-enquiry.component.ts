...
export class ModeOfEnquiryComponent implements OnInit {
  ...
  this.request.addModeofenquiry(this.registerForm.value).subscribe((res: any) => {
  ...
  viewData() {
  this.request.getModeofenquiry().subscribe((response) => {
...
this.request.updateModeofenquiry(this.IdValue,this.editForm.value).subscribe((res : any) => {
    if (res.status == 'Success') {
