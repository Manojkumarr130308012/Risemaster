...
export class FeeTypeComponent implements OnInit {
  ...
  onAddSubmit() {
    ...
    this.request.addFeetype(this.registerForm.value).subscribe(
      (res: any) => {
  ...
  // To display feetype
  viewData() {
    this.request.getFeetype().subscribe(
      response => {
  ...
  onEdit(feetype) {
    ...
    this.request.fetchFeetypeBy(this.Id).subscribe(response => {
      ...
      this.editForm = this.formBuilder.group({
        feetype2: [this.feetypeValue, Validators.required]
      });
  ...
  onEditSubmit() {
    ...
    const edata = {
      feetype: this.editForm.get("feetype2").value
    };
    this.request.updateFeetype(this.IdValue, edata).subscribe(
      (res: any) => {
        if (res.status == "success") {
