...
export class SalutationComponent implements OnInit {
  ...
  onAddSubmit() {
    ...
    this.request.addSalutation(this.registerForm.value).subscribe(
      (res: any) => {
  ...
  viewData() {
    this.request.getSalutation().subscribe(
      response => {
  ...
  onEdit(salutation) {
    ...
    this.request.fetchSalutationBy(this.Id).subscribe(response => {
      ...
      this.editForm = this.formBuilder.group({
        salutation2: [this.salutationValue, Validators.required]
      });
  ...
  onEditSubmit() {
    ...
    const edata = {
      salutation: this.editForm.get("salutation2").value
    };

    this.request.updateSalutation(this.IdValue, edata).subscribe(
      (res: any) => {
        if (res.status == "success") {
