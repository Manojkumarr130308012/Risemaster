...
export class StaffTypeComponent implements OnInit {
  ...
  constructor(
    ...
    this.editForm = this.formBuilder.group({
      stafftype2: ["", Validators.required]
    });
  }
  ...
  onAddSubmit() {
    ...
    this.request.addStafftype(this.registerForm.value).subscribe(
      (res: any) => {
  ...
  viewData() {
    this.request.getStafftype().subscribe(
      response => {
  ...
  onEdit(stafftype) {
    ...
    this.request.fetchStafftypeBy(this.Id).subscribe(response => {
      ...
      this.editForm = this.formBuilder.group({
        stafftype2: [this.stafftypeValue, Validators.required]
      });
  ...
  onEditSubmit() {
    ...
    const edata = {
      stafftype: this.editForm.get("stafftype2").value
    };
    this.request.updateStafftype(this.IdValue, edata).subscribe(
      (res: any) => {
        if (res.status == "success") {
