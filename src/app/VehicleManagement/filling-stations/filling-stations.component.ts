...
export class FillingStationsComponent implements OnInit {
  ...
  onAddSubmit() {
    ...
    this.request.addStation(this.registerForm.value).subscribe(
      (res: any) => {
  ...
  // To display station
  viewData() {
    this.request.getStation().subscribe(
      response => {
  ...
  onEdit(station) {
    ...
    this.request.fetchStationBy(this.Id).subscribe(response => {
      ...
      this.editForm = this.formBuilder.group({
        ...
        contactNo2: [this.contactNoValue, Validators.required]
      });
  ...
  onEditSubmit() {
    ...
    const edata = {
      ...
      contactNo: this.editForm.get("contactNo2").value
    };
    this.request.updateStation(this.IdValue, edata).subscribe(
      (res: any) => {
        if (res.status == "success") {
